...
func (*UnitModel) Find(id int64) (entities.Unit, error) {
	...
	if	err != nil {
		...
		rows, err2 := db.Query("SELECT unit_id, unit_nama, unit_level FROM unit WHERE unit_id = ?", id)
		if err2 != nil {
...
func (*UnitModel) Update(unit entities.Unit) bool {
	...
	if	err != nil {
		...
		result, err2 := db.Exec("UPDATE unit SET unit_nama = ?, unit_level = ? WHERE unit_id = ?", unit.UnitNama, unit.UnitLevel, unit.IdUnit)
		if err2 != nil {
			return false
		} else {
			rowsAffected, _ := result.RowsAffected()
